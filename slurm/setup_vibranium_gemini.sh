...
echo "  nohup python -m experiments.run.run_phase1 gemini-3-flash > logs/gemini-flash.log 2>&1 &"
echo "  nohup python -m experiments.run.run_phase1 gemini-3-pro > logs/gemini-pro.log 2>&1 &"
echo ""
echo "  # Or run all 10 seeds in parallel:"
echo "  for model in gemini-3-flash gemini-3-pro; do"
echo "    for seed in 0 1 2 3 4; do"
echo "      nohup python -m experiments.run.run_phase1 \$model --seeds \$seed > logs/\${model}_s\${seed}.log 2>&1 &"
echo "    done"
echo "  done"
echo ""
echo "  # Monitor:"
