...
Across all 10 models and 5 seeds, 5{,}000 candidates were generated in total, of which $3{,}071$ or $61.4\%$ produced valid fitness scores. To characterise the remaining failures we replay every failed candidate's source through the same gate that BLADE applies at runtime: parse the file, check that imports stay within the allowed \texttt{numpy}-only environment, instantiate the algorithm class with \texttt{(budget=100, dim=2)}, and invoke it once on BBOB function~11 (Discus, $d=2$), with a 10-second per-candidate budget. The result lands in one of five categories: \emph{code generation} (no class is produced, or the file does not parse), \emph{import violation} (the code imports a disallowed package), \emph{interface mismatch} (\texttt{\_\_init\_\_(budget, dim)} or \texttt{\_\_call\_\_(func)} signature mismatch), \emph{runtime error} (compiles and instantiates, but raises during the trial call), and \emph{unclassified}. The same classifier is reused in \cref{sec:results-stage4-failures}, so the two failure-mode figures share categories, colours, and methodology.

The five categories are distributed unevenly across models. Runtime errors are the most common failure mode at 42.1\% of the $1{,}914$ classified events, and they dominate every model that successfully clears the structural gates (Granite-3B, Qwen3.5-4B, Qwen3.5-27B, OLMo-7B, OLMo-32B, and Gemini-3 Pro). Code-generation failures (26.9\%) concentrate sharply on two models: RNJ-1 fails this way in 160 of its 173 failures (92.5\%), never producing a Python class at all, and Qwen3.5-9B contributes 244 further code-generation events, so over three quarters of all such failures come from a pair of models. Import violations (13.3\%) are concentrated in Qwen3.5-4B (96 events) and Devstral-24B (89), models that repeatedly reach for libraries beyond the allowed \texttt{numpy}-only environment. Interface mismatches (10.7\%) spread across the smaller Qwen and Granite models, where \texttt{\_\_init\_\_} or \texttt{\_\_call\_\_} signatures drift from the framework contract. Finally, unclassified failures (7.1\%) concentrate on Gemini-3 Pro (25 of its 51 failures) and Qwen3.5-27B (35 of 205); these models occasionally produce code that fails on specific MA-BBOB instances or dimensions rather than along any of the four structural axes. The two Gemini models retain the lowest overall failure rates, and their failures break down differently: Pro is dominated by runtime and instance-specific failures, whereas Flash fails only ten times in $500$ candidates and never on a structural axis.
...
The Stage~4 dataset contains $16{,}538$ valid candidates produced by Gemini~3 Flash across four feedback conditions (vanilla, neutral-behavioural, SAGE, combined) at 500 candidates per seed and 10 seeds per condition, evaluated on 20 disjoint MA-BBOB functions with five instance seeds each. This section reports the head-to-head comparison: aggregate condition performance, failure-rate dynamics, and the identity of the best-found algorithm in each condition.
...
\Cref{fig:phase4-final-aocc} and \cref{tab:phase4-final-aocc} show the final best-so-far AOCC distribution across 10 seeds per condition. All three feedback-augmented conditions produce a higher mean than vanilla. Sorted by mean, vanilla sits lowest at $0.915 \pm 0.048$, followed by neutral at $0.931 \pm 0.036$, combined at $0.937 \pm 0.009$, and SAGE at $0.938 \pm 0.038$. The spread also shrinks substantially under combined, whose seed-to-seed standard deviation is roughly five times smaller than vanilla's.

A more informative view of the same data is the per-seed final AOCC. Vanilla has four seeds finishing below $0.88$. In comparison, only one neutral seed and one SAGE seed land below this threshold, and combined has none. The three feedback-augmented conditions are less likely to produce a seed that ends in poor-performing code, suggesting that additional feedback signals help the LLM escape suboptimal-code attractors that vanilla otherwise gets stuck in.

\Cref{fig:phase4-convergence} traces the best-so-far AOCC across the 500-candidate budget. Within the first 100 generations, vanilla, SAGE, and combined all reach a mean of roughly $0.90$, with neutral lagging at $0.87$. Vanilla's gen-100 mean of $0.901$ already exceeds the $0.862$ that Gemini-3 Flash reached after 100 candidates on the Stage~1 instance subset. From generation 100 onward the trajectories diverge. Vanilla plateaus near $0.91$, while neutral keeps climbing and nearly matches SAGE and combined by the end of the run. The three feedback signals therefore differ more in tempo than in eventual performance, with SAGE and combined acting early and neutral acting gradually.
...
\Cref{fig:phase4-per-instance} drills the same comparison down to per-instance behaviour, showing the mean AOCC of each condition's best-found algorithm on each of the 20 MA-BBOB functions. The three feedback-augmented conditions split the leaderboard between them: neutral wins on 9 of 20 instances, combined on 6, SAGE on 4, and vanilla on 1. No single condition is uniformly best, but each of the three feedback conditions wins a sizeable share of the benchmark whereas vanilla wins almost none.

To test whether the means separate, we run a Welch one-sided $t$-test of each feedback condition against vanilla. The test returns $p = 0.088$ for combined, $p = 0.126$ for SAGE, and $p = 0.203$ for neutral; under Holm correction across the three pairs no test reaches $\alpha = 0.05$, so the mean lift from feedback sits within sampling noise at the 10-seed budget. The seed-level performance spread, however, is significant. A Levene test for equal variance between vanilla and combined returns $T = 5.57,\, p = 0.030$, confirming that combined's gain over vanilla is a tighter distribution rather than a reliably higher mean.
...
\Cref{tab:phase4-winners} reports the best-found algorithm per condition, taken as the candidate with the highest final best-so-far AOCC across the 10 seeds. The four winners (\texttt{IGA\_CMA}, \texttt{BIPOP\_MA\_CMA\_ES}, \texttt{CMA\_EES}, and \texttt{OM\_CMA\_EIS}) span only a $0.0031$ AOCC range, are each between 95 and 115 lines of code, and have a maximum nesting depth 6. The full source for all four is available in the project repository.\footnote{\url{https://github.com/Maxwe11h/thesis-experiments/tree/main/docs/stage4_winners}} The best-performing algorithm of every condition is a CMA-ES variant, regardless of the feedback string Gemini~3 Flash sees. This is an indication of bias inherent to the model but does not necessarily reflect a flaw, since CMA-ES is genuinely state of the art on this class of benchmark.

Reading the four sources confirms the family bias in detail. Every winner contains all the canonical CMA-ES mechanisms. These are log-decreasing recombination weights, the standard learning-rate constants $c_c, c_s, c_1, c_\mu, d_{\text{amps}}, \chi_n$ verbatim, both evolution paths $p_c$ and $p_s$, the Heaviside $h_\sigma$ indicator, the rank-1 + rank-$\mu$ covariance update, and CSA step-size control. None of the winners replaces, simplifies, or fundamentally re-derives this core; each decorates it with two or three heuristic add-ons.

Three add-ons recur. Mirrored sampling (antithetic $\pm z$ pairs to halve sampling noise on the mean shift) appears in all four winners. QR-decomposed orthogonal sampling also appears in all four, applied to a randn matrix and rescaled by $\sqrt{n}$ (low-dim-only in \texttt{BIPOP\_MA\_CMA\_ES}). Active CMA with negative rank-$\mu$ weights appears in only two of the four. \texttt{IGA\_CMA} uses an aggressive flat $-0.8$ negative coefficient; \texttt{BIPOP\_MA\_CMA\_ES} caps it at $0.5\,c_\mu$. The conditions diverge on the remaining heuristic interventions:

\begin{itemize}
  \item \texttt{IGA\_CMA} (vanilla) wraps the inner CMA in an IPOP-style restart loop that doubles the population size on collapse. A DE-style mean-injection step overrides $x_{\text{mean}}$ on stagnation. Biased re-initialisation draws the new mean near the running best with probability $0.3$.
  \item \texttt{BIPOP\_MA\_CMA\_ES} (neutral) scaffolds BIPOP, alternating between large- and small-population regimes on each restart. The budget accounting that normally selects the next regime is omitted. An unused \texttt{success\_history} list and the absence of any local-search make the ``MA'' (memetic) prefix decorative.
  \item \texttt{CMA\_EES} (SAGE) adds edge-biased radial scaling on the orthogonal samples (a $\text{Beta}(0.5, 0.5)$ mixed into the $\chi$-scale). Modulo-reflective boundary handling bounces out-of-bounds points back into the interior. Restart is conservative. It fires only on near-total stagnation and is suppressed past $80\%$ of the FE budget.
  \item \texttt{OM\_CMA\_EIS} (combined) injects a momentum term, formed as an exponential moving average of recent mean-shift directions, into each sample. This biases offspring along the recent successful trajectory. There is no proper restart, only a $1.1\times$ sigma kick when the within-generation fitness range collapses.
\end{itemize}

A close reading of the four sources also surfaces several patches of code that appear to be functionally inert or technically inconsistent. The mirrored-and-orthogonalised samples in \texttt{BIPOP\_MA\_CMA\_ES}, \texttt{CMA\_EES}, and \texttt{OM\_CMA\_EIS} break the Gaussian assumption that CSA's expected step-norm $\chi_n$ relies on. Bare \texttt{except} clauses in three of the four winners silently fall back to an identity covariance on numerical failure. Comments routinely advertise techniques the code does not implement. A ``noise-resilient gradient estimation'' is named but never coded, a renamed ``PSA'' is just standard CSA, and ``Constraint-based scaling'' is applied to a flat heuristic cap. The LLM-coined acronyms ``EES'', ``EIS'', and the unexplained ``IGA'' prefix make the algorithms sound more distinct than they are.

Beneath these tweaks and labels, every condition runs the same canonical CMA-ES. Feedback shapes the surface heuristics and rhetoric of the winning algorithms but does not move the model off this default. In this experimental design, behavioural feedback shapes appearance and improves final convergence but not identity.
...
\Cref{tab:phase4-fullsuite-summary} reports the mean AOCC per (algorithm, dimensionality) cell. All four LLM-generated algorithms beat the CMA-ES baseline across $d \in \{5, 10, 20\}$. In the empirical attainment function (\cref{fig:phase4-fullsuite-ecdf}), the CMA-ES baseline diverges downward from the LLM variants before $2{,}000$ evaluations and plateaus earliest. \texttt{OM\_CMA\_EIS} (combined) also plateaus, while the other three variants continue improving until the end of the FE budget. \texttt{IGA\_CMA} (vanilla) narrowly leads at $d = 5$ at $0.848$, while \texttt{BIPOP\_MA\_CMA\_ES} (neutral) takes the lead at $d = 10$ ($0.810$) and $d = 20$ ($0.745$). These results validate the neutral feedback format as a viable design for producing high-quality metaheuristics with code-generating LLMs, with its winner leading at the two larger dimensionalities and remaining within $0.015$ AOCC of the leader at $d=5$.