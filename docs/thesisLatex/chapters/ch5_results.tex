...
\begin{figure*}[t]
  \centering
  \includegraphics[width=\textwidth]{figures/fig_feature_sensitivity_combined.pdf}
...
\subsection{Algorithmic Family Distribution}
\label{sec:results-strategy-bias}

\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{figures/fig_strategy_heatmap.pdf}
  \caption{Algorithmic family prevalence by feedback format across all $14{,}127$ valid Stage~3 algorithms, detected by regular expressions on the generated source. Each algorithm may match multiple families. The full pattern set is in \cref{app:strategy-regex}.}
  \label{fig:strategy-heatmap}
\end{figure}

We classify each valid Stage~3 algorithm into thirteen algorithmic families using regular expressions on the source; algorithms may match multiple families. \Cref{fig:strategy-heatmap} separates the families into three bands. Surrogate/Bayesian methods, multi-population schemes, and simulated annealing are absent across all $14{,}127$ algorithms regardless of format. CMA-ES, BIPOP/restart, and adaptive step-size rules appear at $50$--$62\%$ in every format with no clear ordering between them. Population-based code, Differential Evolution, and Genetic Algorithm operators decrease monotonically from neutral to comparative ($94 \to 53\%$, $78 \to 54\%$, $61 \to 38\%$).

The empty band reflects the generator: Gemini~3's training prior excludes three established families regardless of feedback. The monotone band mirrors \cref{sec:results-ast}: neutral stacks DE and GA operators on top of the shared CMA-ES core, producing the hybrid code the AST analysis flagged. Algorithm family thus restates the complexity gap rather than acting as an independent differentiator, and the qualitative results in this section remain conditional on the chosen LLM.

% ===========================================================================
\section{Full-Benchmark Comparison}
\label{sec:results-stage4}

The Stage~4 dataset comprises $20{,}000$ candidate algorithms generated by Gemini~3 Flash across four feedback conditions (vanilla, neutral-behavioural, SAGE, combined) at 500 candidates per seed and 10 seeds per condition, evaluated on 20 disjoint MA-BBOB functions with five instance seeds each. This section reports the head-to-head comparison: aggregate condition performance, per-instance robustness, failure-rate dynamics, behavioural steering at scale, and the identity of the best-found algorithm in each condition.
...
\Cref{tab:phase4-winners} reports the best-found algorithm per condition, taken as the candidate with the highest final best-so-far AOCC across the 10 seeds. The four winners span only a $0.0031$ AOCC range, and all four are CMA-ES variants. Their source identifiers (\texttt{IGA\_CMA}, \texttt{BIPOP\_MA\_CMA\_ES}, \texttt{CMA\_EES}, \texttt{OM\_CMA\_EIS}) and code structure (95--115 lines, depth-6 nesting) match the family saturation pattern established in \cref{sec:results-strategy-bias}: at the very top of every condition, Gemini~3 Flash returns the same family of algorithm regardless of the feedback string. The full source for each winner together with a structured family-classification prompt is preserved alongside the analysis output.\footnote{\texttt{analysis/figs\_phase4/p4\_winners/}, with one \texttt{<cond>\_winner.py} and one \texttt{<cond>\_prompt.txt} per condition.}

The implication for Stage~4 mirrors the Stage~3 saturation argument. The aggregate AOCC differences between conditions in \cref{sec:results-stage4-agg} cannot be attributed to a richer or more diverse algorithm family under feedback-augmented conditions, since the family is fixed across all four. Differences must instead arise from how reliably each condition steers the search through the CMA-ES design space. Whether the small mean differences observed on the twenty-function set translate to a larger gap on the full $1{,}000$-function MA-BBOB pool (and against an external CMA-ES baseline) is the question the generalisation experiment in \cref{sec:results-stage4-generalisation} addresses.

% ---------------------------------------------------------------------------
\subsection{Full-Suite Generalisation}
\label{sec:results-stage4-generalisation}

The 20-function set used above is large enough to discriminate condition means at the budget set by 10 seeds, but it is sized to support the head-to-head comparison rather than to estimate generalisation across the full MA-BBOB pool. To position the four winners against the broader benchmark, the best-found algorithm from each condition (\cref{tab:phase4-winners}) plus a CMA-ES~\cite{hansen2001cmaes} baseline are evaluated on all $1{,}000$ MA-BBOB functions $\times$ 5 instance seeds $\times$ \{5, 10, 20\} dimensions under a $2{,}000d$ FE budget, with no LLM in the loop. The total compute is approximately $1.75 \times 10^9$ function evaluations across the five algorithms. Results are pending at the time of writing and will be reported in the final revision.