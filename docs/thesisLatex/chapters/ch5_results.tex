...
To position the four winners against a broader benchmark, the best-found algorithm from each condition (\cref{tab:phase4-winners}) is re-evaluated against two external baselines, with no LLM in the loop: a hand-tuned CMA-ES~\cite{hansen2001cmaes}, and \texttt{NeighborhoodAdaptiveDE} (NADE)~\cite{vanStein2025NeighborhoodDE}, the winner of the 2025 MA-BBOB competition. NADE is itself an LLaMEA-generated algorithm, produced with Gemini~2.0~Flash, and, like every algorithm in this thesis, was designed and tuned only on 5-dimensional MA-BBOB (\cref{sec:meth-benchmark}); the higher dimensionalities therefore probe how each algorithm generalises beyond its design regime. Each of the six algorithms runs on all $1{,}000$ MA-BBOB functions across 5 instance seeds and three dimensionalities $d \in \{5, 10, 20\}$, under a $2{,}000d$ FE budget per run. The total compute is approximately $2.1 \times 10^9$ FEs across the six algorithms.

\begin{table}[t]
\centering
\caption{Mean AOCC per (algorithm, dimensionality) on the full $1{,}000$-instance MA-BBOB pool, averaged over 5 seeds ($5{,}000$ runs per cell). Best entry per dimensionality in bold. The two \texttt{baseline} rows are external references with no LLM in the loop; NADE denotes \texttt{NeighborhoodAdaptiveDE}, the 2025 MA-BBOB competition winner.}
...
baseline  & NADE~\cite{vanStein2025NeighborhoodDE} & 0.810 & 0.663 & 0.471 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure*}[t]
  ...
  \caption{Empirical attainment function (EAF) on the full $1{,}000$-instance MA-BBOB pool at $d \in \{5, 10, 20\}$. The x-axis is the function-evaluation budget (up to $2000 \cdot d$). The y-axis is the fraction of $(\text{run}, \text{target})$ pairs for which the run's best-so-far fitness has fallen below the target by that budget, taken over $5{,}000$ runs ($1{,}000$ MA-BBOB instances $\times$ 5 seeds) and 51 log-spaced targets in $[10^{-8}, 10^2]$. Top row: full $[0, 1]$ EAF range. Bottom row: zoomed to $[0.6, 1]$ to resolve the plateau differences between algorithms. The two dashed lines mark the external baselines (CMA-ES and NADE). The four LLM-generated variants overlap to within line-width and dominate the early and middle budget at every dimensionality. NADE converges slowly: at $d = 5$ it starts below every other algorithm but overtakes them late to reach the highest final coverage; at $d = 10$ it again starts slowest and finishes level with CMA-ES; at $d = 20$ it trails CMA-ES throughout.}
  \label{fig:phase4-fullsuite-ecdf}
\end{figure*}

\Cref{tab:phase4-fullsuite-summary} reports the mean AOCC per (algorithm, dimensionality) cell. On this anytime metric all four LLM-generated winners beat both external baselines at every dimensionality: \texttt{IGA\_CMA} (vanilla) leads at $d = 5$ ($0.848$), and \texttt{BIPOP\_MA\_CMA\_ES} (neutral) at $d = 10$ ($0.810$) and $d = 20$ ($0.745$). CMA-ES degrades gently with dimension ($0.728 \to 0.701 \to 0.650$), while NADE's mean AOCC is lower and falls faster ($0.810 \to 0.663 \to 0.471$), dropping below CMA-ES from $d = 10$ onward. The attainment curves (\cref{fig:phase4-fullsuite-ecdf}) qualify this ranking at low dimension: NADE is a slow starter that ramps late, so at $d = 5$ it begins below every other algorithm yet overtakes them to the highest final coverage, and at $d = 10$ it finishes level with CMA-ES. Its low AOCC at these dimensions therefore reflects slow convergence rather than weak final solutions, consistent with its standing as the 5-dimensional competition winner; only at $d = 20$ does it fail outright, trailing CMA-ES across the whole budget. Among the winners, \texttt{OM\_CMA\_EIS} (combined) plateaus early while the other three keep improving to the end of the budget.

This profile is informative because NADE shares the winners' provenance: it too was produced by LLaMEA, with Gemini~2.0~Flash, and tuned only on 5-dimensional MA-BBOB. The four winners nonetheless converge faster at every dimension and, unlike NADE, hold their final quality as dimension grows, and the reason is visible in the generated code. Every winner converged on a dimension-adaptive CMA-ES template, with a population that grows with dimension (around $4 + 3\ln d$) and full covariance adaptation, whereas NADE is a fixed-population differential-evolution variant whose $50$-member population and stagnation-triggered restarts neither accelerate convergence nor scale to higher dimension. Dimensional robustness is therefore not an automatic property of LLM-designed algorithms; it follows from the structural template the model settles on, and the CMA-ES family favoured by Gemini~3~Flash (\cref{sec:results-stage4-winners}) happens to carry it.
