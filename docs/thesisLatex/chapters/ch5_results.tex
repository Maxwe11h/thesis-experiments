...
The implication for Stage~4 is that the AOCC differences between conditions in \cref{sec:results-stage4-agg} cannot be attributed to a richer or more diverse algorithm family under feedback-augmented conditions. The family is fixed across all four winners; the differences are restricted to which heuristic decorations the LLM bolts onto the same CMA-ES skeleton. Whether the $0.022$--$0.023$ AOCC gap observed on the twenty-function set holds up on the full $1{,}000$-function MA-BBOB pool against an external CMA-ES baseline is the question the generalisation experiment in \cref{sec:results-stage4-generalisation} addresses.
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\textwidth]{figures/fig_phase4_fullsuite_final_aocc.pdf}
  \caption{Final-AOCC distribution per (algorithm, dimensionality) across all $5{,}000$ runs per cell ($1{,}000$ instances $\times$ 5 seeds). Boxes show inter-quartile range, dark lines the median, whiskers the 1.5 IQR fences. The long lower whiskers reflect the heavy tail of MA-BBOB: a fraction of instances are genuinely hard for every algorithm tested.}
  \label{fig:phase4-fullsuite-final-aocc}
\end{figure}

\begin{figure}[t]
  \centering
  \includegraphics[width=\textwidth]{figures/fig_phase4_fullsuite_ecdf.pdf}
  \caption{Empirical attainment-function-based ECDF curves on the full $1{,}000$-instance MA-BBOB pool at $d \in \{5, 10, 20\}$. Each curve aggregates 51 log-spaced fitness targets in $[10^{-8}, 10^2]$ across $5{,}000$ runs per algorithm. The dashed line marks the external CMA-ES baseline; the four LLM-generated variants overlap to within line-width and trail the budget by a stable margin above it.}
  \label{fig:phase4-fullsuite-ecdf}
\end{figure}

\Cref{tab:phase4-fullsuite-summary} reports the mean AOCC per (algorithm, dimensionality) cell, with the corresponding distributional view in \cref{fig:phase4-fullsuite-final-aocc}. Three observations follow. First, all four LLM-generated algorithms beat the CMA-ES baseline by a margin of $0.10$--$0.12$ mean AOCC, uniformly across $d \in \{5, 10, 20\}$. The smallest gap, $0.10$ at $d = 20$, is still larger by an order of magnitude than the $0.022$--$0.023$ within-condition spread observed in the head-to-head comparison of \cref{sec:results-stage4-agg}. Second, the ranking within the LLM group is nearly indistinguishable: \texttt{IGA\_CMA} (vanilla) narrowly leads at $d = 5$ at $0.848$, while \texttt{BIPOP\_MA\_CMA\_ES} (neutral) takes the lead at $d = 10$ ($0.810$) and $d = 20$ ($0.745$). The total spread among the four LLM winners is $0.03$--$0.05$ AOCC across the three dimensionalities --- close to the $\pm 0.005$ noise floor implied by the heavy-tailed distributions in \cref{fig:phase4-fullsuite-final-aocc}. Third, the head-to-head winner of \cref{sec:results-stage4-agg} (combined) does not reproduce its lead on the broader benchmark: it ranks fourth at $d = 5$, third at $d = 10$, and second at $d = 20$. The $0.022$--$0.023$ AOCC advantage of the combined condition observed on the twenty-function set therefore does not transfer to the full pool.

\Cref{fig:phase4-fullsuite-ecdf} extends the same observations onto the per-FE attainment dimension. The CMA-ES baseline tracks the LLM variants closely through the early budget but separates from them past $\mathrm{FE}/d \approx 30$, where the LLM variants' final-stage exploitation pays off. The four LLM ECDF curves themselves are visually indistinguishable across all three dimensionalities, consistent with the boxplot view: the within-LLM ranking that \cref{sec:results-stage4-agg} relies on is not stable under generalisation.
