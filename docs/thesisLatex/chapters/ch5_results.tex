...
\begin{figure*}[t]
  ...
  \caption{Empirical attainment function (EAF) on the full $1{,}000$-instance MA-BBOB pool at $d \in \{5, 10, 20\}$. Each curve aggregates 51 log-spaced fitness targets in $[10^{-8}, 10^2]$ across $5{,}000$ runs per algorithm; the y-axis reports the fraction of (run, target) pairs satisfied by a given function-evaluation budget. Top row: full $[0, 1]$ EAF range. Bottom row: zoomed to $[0.5, 1]$ to resolve the plateau differences between algorithms. The dashed line marks the external CMA-ES baseline; the four LLM-generated variants overlap to within line-width and sit above it by a stable margin throughout the budget.}
