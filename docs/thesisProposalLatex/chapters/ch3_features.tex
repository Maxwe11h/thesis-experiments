...
in this thesis.  Of those 23, six are entirely novel metrics that have not
appeared in prior optimisation literature.  These novel features constitute a
central contribution of this work: they capture aspects of search dynamics ---
momentum, burstiness, plateau structure, and dimensional heterogeneity, that
existing metrics overlook.
...
\begin{enumerate}
  ...
  \item \textbf{Discrimination.} Features should separate high-performing from low-performing algorithms. We quantify this via Spearman correlation with AOCC and Kolmogorov--Smirnov effect size between the top-25\% and bottom-25\% candidates.
\end{enumerate}
...
al.~\cite{vanStein2025Behaviour, vanStein2025BLADE}.  We group them into four
functional categories.

\paragraph{Exploration and diversity.}
\begin{itemize}
  \item \texttt{avg\_nearest\_neighbor\_distance}: mean Euclidean distance between each evaluated point and its predecessor, quantifying how much the search covers new territory at each step.
  \item \texttt{dispersion}: maximum gap in search-space coverage (via KD-tree). Large dispersion indicates unvisited regions.
  \item \texttt{avg\_exploration\_pct}: chunk-based measure of within-chunk variance relative to a random baseline. Its complement \texttt{avg\_exploitation\_pct} is also recorded.
\end{itemize}
...
\begin{itemize}
  ...
  \item \texttt{intensification\_ratio}: fraction of evaluations within radius $r = 0.1 \times (\mathit{ub} - \mathit{lb})$ of the final best, defined as $I_r = T^{-1}\sum_{t}\mathbf{1}[\lVert x_t - x_{\mathrm{best}} \rVert < r]$. High values indicate strong local exploitation.
  \item \texttt{avg\_exploitation\_pct}: complement of exploration percentage.
\end{itemize}

\paragraph{Convergence.}
\begin{itemize}
  \item \texttt{average\_convergence\_rate}: geometric mean of successive best-so-far error ratios.
  \item \texttt{avg\_improvement}: mean fitness improvement on steps that update the best-so-far, $\bar{\Delta}_f = |\mathcal{I}|^{-1}\sum_{t \in \mathcal{I}}(f^*_{t-1} - y_t)$, where $\mathcal{I} = \{t : y_t < f^*_{t-1}\}$.
  \item \texttt{success\_rate}: fraction $|\mathcal{I}| / T$ of evaluations that improve the best-so-far.
\end{itemize}
...
al.~\cite{cenikj2026features} We include them because BLADE does not compute
them.

Let $s_t = \lVert x_{t+1} - x_t \rVert$ denote the Euclidean step size at
evaluation~$t$.
...
population-based algorithm with many individuals per generation.  In our
...
category.  Each candidate algorithm is evaluated on 50 MA-BBOB trajectories
(10 problem instances $\times$ 5 seeds), so per-candidate cost is $50\times$
the per-trajectory cost. The dominant cost is the existing BLADE metrics (which build a KD-tree for
dispersion) and sample entropy (which is $O(N^2)$ even after subsampling).
The 17 features from categories 1, 4, and~5 add under 100\,ms per candidate in
total.

\begin{table}[t]
  \centering
  \caption{Computational cost of behavioural feature extraction.  All timings
    are measured on a single CPU core for $T = 10{,}000$ evaluations in $d = 5$
    dimensions.  SampEn uses subsampled trajectories ($N = 1{,}000$).}
  \label{tab:feature_cost}
  \footnotesize
  \begin{tabular}{@{}lrrr@{}}
    \toprule
    Category & \# & Per traj. & Per cand. \\
    \midrule
    Existing (BLADE)            & 11 & $\sim$1--2\,s   & $\sim$1--2\,min \\
    Step-size dynamics          &  4 & $\sim$0.6\,ms   & $\sim$30\,ms    \\
    Information-theoretic       &  4 & $\sim$0.6\,s    & $\sim$30\,s     \\
    Adapted pop.\ dynamics      &  7 & $\sim$0.5\,ms   & $\sim$25\,ms    \\
    Novel features              &  6 & $\sim$0.9\,ms   & $\sim$45\,ms    \\
    \midrule
    \textbf{Total}              & \textbf{32} & $\sim$\textbf{1.6--2.6\,s} & $\sim$\textbf{1.5--2.5\,min} \\
    \bottomrule
  \end{tabular}
\end{table}

Relative to the MA-BBOB evaluation itself (30--120\,s per candidate to run
the algorithm 50 times), the total feature extraction overhead of 1.5--2.5
minutes represents a modest addition.  Over a full experimental run of 100
candidates, this amounts to roughly 2.5--4 hours of feature extraction
compared to 1--3 hours of algorithm evaluation, a ratio that is acceptable
given the diagnostic value the features provide.
